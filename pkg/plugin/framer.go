...
func NewColumn(col *pb.ColumnSpec, name string, alias string, kind string, labels data.Labels) column {
	...
	case *pb.TypeSpec_Basic_:
		...
		case pb.TypeSpec_DATE:
			field := data.NewField(col.Name, labels, []*time.Time{})
			field.Config = config
			return column{
				field,
				dateTimeConverter,
		...
		case pb.TypeSpec_TEXT, pb.TypeSpec_VARCHAR:
			field := data.NewField(col.Name, labels, []*string{})
			field.Config = config
			return column{
				field,
		...
		case pb.TypeSpec_DECIMAL:
			field := data.NewField(col.Name, labels, []*float64{})
			field.Config = config
			return column{
				field,
		...
		case pb.TypeSpec_INT:
			field := data.NewField(col.Name, labels, []*int64{})
			field.Config = config
			return column{
				field,
		...
		case pb.TypeSpec_BOOLEAN:
			field := data.NewField(col.Name, labels, []*bool{})
			field.Config = config
			return column{
				field,
